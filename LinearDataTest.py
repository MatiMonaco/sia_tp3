...
import numpy as np
entries = np.genfromtxt("train_set.txt", delimiter=",", skip_header=1)
z = np.genfromtxt("expected_outputs.txt", delimiter=" ", skip_header=1)

entries2 = np.array(([-1, 1], [1, -1], [-1, -1], [1, 1]))
entries_len = len(entries2)
z2 = np.array(([-1, -1, -1, 1]))
slp = SimpleLinearPerceptron()
result = slp.fit(0.2,entries,z,0.1,0.3)
